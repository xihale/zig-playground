...
const HOOK_PATH = process.env.ZP_HOOK_PATH ?? "/hooks/zp-deploy";
// Repo currently uses master; accept both so a rename is a non-event.
const DEPLOY_BRANCHES = new Set(["master", "main"]);
const NTFY_CONFIG = process.env.ZP_NTFY_CONFIG ?? "/home/zig-ci/.ntfy-notify";
const NTFY_URL = process.env.ZP_NTFY_URL ?? "https://ntfy.xeed.ink";
...
const child = spawn("/bin/bash", [DEPLOY_SH, ref], {
  ...
  env: { ...process.env, ZP_PUSH_SHA: sha },
});
