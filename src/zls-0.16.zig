//! WASM entry for ZLS 0.16.x (std.Io + nested window.MessageType).
//! 0.15.x still uses src/zls.zig via the playground build.
...
fn logFn(
    ...
    transport.writeJsonMessage(io, json_message) catch {};
}
...
fn readJsonMessage(
    ...
    _: std.Io,
    _: std.mem.Allocator,
) (std.mem.Allocator.Error || zls.lsp.Transport.ReadError)![]u8 {
    unreachable;
}
...
) zls.lsp.Transport.WriteError!void {
...
export fn createServer() void {
    ...
    config_manager = zls.configuration.Manager.init(io, allocator, &environ_map) catch
        @panic("config manager init failed");
