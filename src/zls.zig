...
fn writeJsonMessage(_: *zls.lsp.Transport, json_message: []const u8) zls.lsp.Transport.WriteError!void {
    // The only failure mode here is OOM appending to the in-memory output buffers.
    // Map it onto a member of WriteError (std.posix.WriteError) so the vtable signature is
    // satisfied; NoSpaceLeft is present in std.posix.WriteError in 0.15.2.
...
export fn createServer() void {
    // config_manager defaults to null; Server.create initializes it internally
    // via configuration.Manager.init(allocator). This mirrors ZLS 0.15.1's own main.zig.
